...
const BasicProfileTab: React.FC<BasicProfileTabProps> = ({
  ...
  // Owner: view mode by default (premium display) — form only when editing
  if (isOwner && !isEditing) {
    return (
      <>
        ...
        />
        {!hasProperty && (
          <section className="mt-4 rounded-2xl bg-card border border-border/60 shadow-sm p-5 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Add your property first to get your Owner ID. Renters use this ID
                  to connect with you.
                </p>
              </div>
            </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Avatar-only header for renter (kept unchanged) or owner edit mode */}
        ...
        <div className="relative p-6 flex flex-col items-center text-center gap-4">
          ...
          <div className="space-y-1">
            <h2 className="text-xl font-bold text-foreground leading-tight">
              {formValues.fullName || (isOwner ? 'Your Name' : 'Your Profile')}
            </h2>
            {user?.email && (
