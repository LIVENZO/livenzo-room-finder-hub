...
const ProfileCompletionBanner: React.FC<ProfileCompletionBannerProps> = ({
  ...
  return <Alert className="bg-amber-50 border-amber-200 mb-6">
      <AlertCircle className="h-4 w-4 text-amber-600" />
      <AlertTitle className="text-amber-700">Profile Incomplete</AlertTitle>
      <AlertDescription className="text-amber-600">
        {!basicComplete ? <>Please complete your name and phone number to unlock account features.</> : isOwner && !ownerComplete && <>Property details are optional and only required when listing rooms or managing connections.</>}
      </AlertDescription>
    </Alert>;
};
