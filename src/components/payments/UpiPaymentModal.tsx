...
export const UpiPaymentModal = ({
    ...
    const fetchOwnerDetails = async () => {
      try {
        ...
        const {
          data: relationship,
          error: relError
        } = await supabase.from('relationships').select('owner_id').eq('id', relationshipId).single();
        ...
        const {
          data: profile,
          error: profileError
        } = await supabase.from('user_profiles').select('upi_phone_number').eq('id', relationship.owner_id).single();
        if (profileError) throw profileError;
        setOwnerUpiPhone(profile.upi_phone_number || "Not available");
      } catch (error) {
        console.error('Error fetching owner details:', error);
        toast({
          description: "Failed to load owner payment details",
          variant: "destructive"
        });
      }
    };
  ...
  const handleCopyUpiPhone = async () => {
    try {
      ...
      setCopied(true);
      toast({
        description: "UPI Phone Number copied to clipboard!"
      });
      setTimeout(() => setCopied(false), 2000);
    } catch (error) {
      toast({
        description: "Failed to copy UPI Phone Number",
        variant: "destructive"
      });
    }
  };
    ...
    try {
      ...
      if (proofFile) {
        ...
        const {
          data: uploadData,
          error: uploadError
        } = await supabase.storage.from('user-uploads').upload(fileName, proofFile);
      ...
      } = await supabase.from('payments').select('id').eq('renter_id', user?.id).eq('owner_id', ownerId).eq('relationship_id', relationshipId).eq('billing_month', billingMonth).maybeSingle();
      if (existingPayment) {
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-2">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 border rounded-lg">
              ...
              <Button variant="outline" size="sm" onClick={handleCopyUpiPhone} className="ml-2" disabled={!ownerUpiPhone || ownerUpiPhone === "Not available"}>
                {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
              </Button>
            </div>

            {/* Pay via UPI App Button */}
            <Button onClick={handlePayWithUpi} className="w-full" size="lg">
          ...
          <div className="space-y-3 pt-4 border-t">
            ...
            <div className="space-y-2">
              ...
              <Input id="transactionId" placeholder="Enter UPI transaction ID" value={transactionId} onChange={e => setTransactionId(e.target.value)} />
            </div>

            <div className="space-y-2">
              
              <div className="flex items-center gap-2">
                <Input id="proof" type="file" accept="image/*" onChange={handleFileUpload} className="hidden" />
        ...
        <div className="flex-shrink-0 space-y-3 border-t pt-4">
          <div className="flex gap-3">
            ...
            <Button onClick={handleSubmitPayment} disabled={loading || !transactionId.trim()} className="flex-1">
