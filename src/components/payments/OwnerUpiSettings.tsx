...
export const OwnerUpiSettings = () => {
  ...
  const handleSave = async () => {
    if (!upiId.trim()) {
      toast({ description: "Please enter a valid UPI ID", variant: "destructive" });
      return;
    }
  ...
  const handleDeleteQr = async () => {
    ...
    try {
      ...
      await supabase.storage
        .from('user-uploads')
        .remove([upiDetails.qr_code_file_name]);
      ...
      toast({ description: "QR code deleted successfully!" });
      fetchUpiDetails();
    } catch (error) {
      console.error('Error deleting QR code:', error);
      toast({ description: "Failed to delete QR code", variant: "destructive" });
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Configure your UPI details for direct payments from renters
        </CardDescription>
      </CardHeader>
