...
-- Step 1: Create a function to handle FCM token upserts with proper conflict resolution
...
-- Step 2: Update the trigger function to use proper token cleanup
DROP TRIGGER IF EXISTS replace_fcm_token_trigger ON public.fcm_tokens;
DROP FUNCTION IF EXISTS public.replace_fcm_token();
